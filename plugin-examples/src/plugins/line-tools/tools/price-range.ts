...
class PriceRangePaneRenderer implements IPrimitivePaneRenderer {
    ...
    draw(target: CanvasRenderingTarget2D): void {
        target.useBitmapCoordinateSpace(scope => {
            ...
            // Position label at top or bottom based on direction
            const labelY = price2 > price1 ? yMax + 25 * scope.verticalPixelRatio : yMin - 10 * scope.verticalPixelRatio;

            // Draw label
            ctx.font = `bold ${14 * scope.verticalPixelRatio}px sans-serif`;
            ctx.fillStyle = this._options.borderColor;
            ctx.textAlign = 'center';
            ctx.textBaseline = price2 > price1 ? 'top' : 'bottom';
